        ...
        <div class="stat-card">
            ...
            <div class="mt-4 pt-4 border-t border-slate-50">
                <a href="{{ route('admin.pelanggan.index') }}"
                    class="text-xs text-blue-600 font-medium hover:text-blue-700 flex items-center gap-1">
                    Lihat semua
        ...
        <div class="stat-card">
            ...
            <div class="mt-4 pt-4 border-t border-slate-50">
                <a href="{{ route('admin.meteran.create') }}"
                    class="text-xs text-yellow-600 font-medium hover:text-yellow-700 flex items-center gap-1">
                    Input meteran
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                    </svg>
            ...
            <div class="mt-4 pt-4 border-t border-red-50">
                @if($stats['tagihan_jatuh_tempo'] > 0)
                    <form method="POST" action="{{ route('admin.tagihan.mark-overdue') }}" class="inline">
                        @csrf
                        <button type="submit"
                            class="text-xs text-red-600 font-medium hover:text-red-700 flex items-center gap-1">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-6">
                ...
                <div id="chart-tagihan" class="h-48 flex items-end gap-2 px-2">
                    @foreach($chartData as $item)
                        ...
                        <div class="flex-1 flex flex-col items-center gap-1.5">
                            <span
                                class="text-xs font-bold text-slate-600">{{ $item['count'] > 0 ? $item['count'] : '' }}</span>
                            <div class="w-full rounded-t-xl relative group cursor-default transition-all duration-300"
                                ...
                                @if($item['count'] > 0)
                                    <div
                                        class="absolute -top-9 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-xs px-2.5 py-1 rounded-lg hidden group-hover:block whitespace-nowrap shadow-lg z-10 pointer-events-none">
        ...
        <div class="section-card">
            ...
            <div class="p-4 space-y-3">
                ...
                {{-- Daftar Pelanggan --}}
                <a href="{{ route('admin.pelanggan.create') }}" class="flex items-center gap-4 p-3.5 rounded-2xl border border-transparent
                    ...
                    <svg class="w-4 h-4 text-slate-300 group-hover:text-blue-400 group-hover:translate-x-0.5 transition-all"
                ...
                {{-- Input Meteran --}}
                <a href="{{ route('admin.meteran.create') }}" class="flex items-center gap-4 p-3.5 rounded-2xl border border-transparent
                    ...
                    <svg class="w-4 h-4 text-slate-300 group-hover:text-green-400 group-hover:translate-x-0.5 transition-all"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                    </svg>
                </a>
                ...
                {{-- Monitor Tagihan --}}
                <a href="{{ route('admin.tagihan.index') }}" class="flex items-center gap-4 p-3.5 rounded-2xl border border-transparent
                    ...
                    <svg class="w-4 h-4 text-slate-300 group-hover:text-amber-400 group-hover:translate-x-0.5 transition-all"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                    </svg>
                </a>
                ...
                {{-- Laporan --}}
                <a href="{{ route('admin.laporan.index') }}" class="flex items-center gap-4 p-3.5 rounded-2xl border border-transparent
                    ...
                    <svg class="w-4 h-4 text-slate-300 group-hover:text-purple-400 group-hover:translate-x-0.5 transition-all"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                    </svg>
                </a>
